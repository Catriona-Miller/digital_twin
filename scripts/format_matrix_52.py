...
pss_data = combine_df(pss_data)
sleep_data = combine_df(sleep_data)
...
fcis_data = fcis_data[['subjectID', 'Total_FCIS_0', 'Total_FCIS_52']]
glitter_data = glitter_data[['subjectID', 'glitter_seconds_0', 'glitter_seconds_52']]
...
sleep_data = sleep_data[['subjectID', 'Bedtime_difficulty_0', 'Night_sleep_0', 'Night_sleep_52', 'Bedtime_difficulty_52']]
...
all_data = [
    ...
    lipids_data, psd_data, pss_data, sanitation_data, sleep_data, vitamin_data]
...
combined_df.to_csv('../data/combined_matrix_large_52.tsv', sep='\t', index=False)
...
categorical_cols = ['Number_of_rooms_in_current_household'] + list(sanitation_data.columns[1:]) + dummy_cols
...
# save both
combined_imputed_scaled.to_csv('../data/combined_imputed_scaled_large_52.tsv', sep='\t', index=True)
joblib.dump(fitted_scaler, '../data/scaler_large_52.save')